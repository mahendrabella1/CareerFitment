...
export default function AdminReportPage({ params }: { params: { uid: string } }) {
  ...
  // Same per-class adaptation Dashboard.tsx/DashboardMobile.tsx do: `a` is
  // the raw latestAssessment document, not the AssessmentSummary shape
  // FullReport actually expects - without running it through the matching
  // class-specific adapter (and, for 11-12, building the Career Fitment/
  // Selector/Suitability extra sheets), every dimension reads as 0% and
  ...
  return (
    <div style={S.reportPage}>
      ...
      {class11Stale || class678Stale ? (
        <Center>
          <p style={SHARED.muted}>
            This report was generated by an earlier version of the assessment and can&apos;t be rendered with the
            current scoring - ask <b>{viewing.name || viewing.email || "the student"}</b> to retake the assessment.
          </p>
          <BackLink />
        </Center>
      ) : (
