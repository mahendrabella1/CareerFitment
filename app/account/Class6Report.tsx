...
const P6 = "https://onegrasp.com/wp-content/uploads/2026/07/";
const DIMS8_6 = P6 + "ChatGPT-Image-Jul-10-2026-05_34_15-PM.png";

// "Developing"/"Emerging" instead of a blunt "Low"/"Weak" — an 11-year-old
// shouldn't be told outright they're "weak" at something in their own report.
const bandOf6 = (p: number) =>
 ...
 *  dimension. */
function class6Radar(output: Class6ScoreOutput): RadarDatum[] {
...
function dimDeepDive6(key: string, output: Class6ScoreOutput): { strengths: string[]; grow: string[]; recommend: string[] } {
  switch (key) {
    case "personality": {
...
function DimDeepDive6({ score, dive }: { score: number; dive: { strengths: string[]; grow: string[]; recommend: string[] } }) {
  const band = bandOf6(score);
  const pct = Math.max(5, Math.min(97, Math.round(score * 0.95)));
  return (
    <div className="c6-deepdive">
      <div className="c6-deepdive-hero">
        <Ring value={score} size={84} stroke={9} color="#F2555A">
          <div className="c6-ring-num">{score}</div>
          <div className="c6-ring-den">/ 100</div>
        </Ring>
        <div className="c6-deepdive-meta">
          <span className={`c6-scorecard-pill ${band.tone}`}>{band.label}</span>
          {score > 0 ? <span className="c6-deepdive-pct">Higher than {pct}% of students your age</span> : null}
        </div>
      </div>
      {(dive.strengths.length > 0 || dive.grow.length > 0) && (
        <div className="c6-twocard">
          <div className="c6-lc good">
            <h4>Where you&rsquo;re strong</h4>
            <ul>{dive.strengths.map((x, i) => <li key={i}>{x}</li>)}</ul>
          </div>
          <div className="c6-lc grow">
      ...
      {dive.recommend.length > 0 && (
        <div className="c6-recos">
          <div className="c6-recos-hd">Recommended next steps</div>
...
export function Class6Report({
  ...
  const radar = class6Radar(output);
    ...
    <div className="class6-report">
      ...
      <section className="c6-sheet c6-cover">
        <div className="c6-cover-content">
          <div className="c6-logo-card">
            <img src="/onegrasp-logo-tight.png" alt="OneGrasp" className="c6-logo" />
          </div>
          <span className="c6-badge">Career discovery · scientifically structured</span>
          ...
          <div className="c6-student-block">
            <span className="c6-label">Prepared for</span>
            <div className="c6-student-name">{studentName}</div>
            <span className={`c6-student-school${institution ? "" : " ph"}`}>{institution || "School name"}</span>
          </div>
          <div className="c6-cover-chips">
            {studentClass ? <span className="c6-chip c6-cc1"><span className="k">Class</span><span className="v">{studentClass}</span></span> : <span className="c6-chip c6-cc1"><span className="k">Class</span><span className="v">6</span></span>}
            <span className="c6-chip c6-cc3"><span className="k">Exam date</span><span className="v">{format(completedDate, "dd MMM yyyy")}</span></span>
          </div>
        </div>
      </section>
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">What this report covers</h2>
          <p className="c6-description">Read it top to bottom, or jump to what matters. Every section is built from your own answers.</p>
          <div className="c6-toc">
            {[
              ...
              <div className="c6-toc-row" key={t}>
                <div className="c6-toc-t">{t}</div>
                <div className="c6-toc-d">{d}</div>
              </div>
            ))}
          </div>
          <div className="c6-dims8">
            <img src={DIMS8_6} alt="The eight dimensions this report measures" loading="lazy" />
          </div>
        </div>
      </section>
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Your eight-dimension scorecard</h2>
          <p className="c6-description">Strongest first. Every score is 0-100, and the tick on each bar marks a typical student at your stage.</p>
          <div className="c6-scorecard-list">
            {radar.slice().sort((a, b) => b.score - a.score).map((d) => {
              const band = bandOf6(d.score);
              return (
                <div className="c6-scorecard-row" key={d.key}>
                  <span className="c6-scorecard-nm">{d.label}</span>
                  <span className="c6-scorecard-bar"><span className="c6-scorecard-fill" style={{ width: `${Math.max(2, Math.min(100, d.score))}%` }} /></span>
                  <span className={`c6-scorecard-pill ${band.tone}`}>{band.label}</span>
                </div>
              );
            })}
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Aptitude &amp; Reasoning</h2>
          <p className="c6-description">
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Your profile, in one page</h2>
          <p className="c6-description">No single answer defines you — the shape of all eight together is what makes this read accurate.</p>
          <div className="c6-radar-hero">
            <div style={{ display: "flex", justifyContent: "center" }}>
              <RadarChart data={radar} color="#F2555A" />
            </div>
            <div className="c6-radar-domains">
              {output.domainAffinities.slice(0, 5).map((d, i) => {
                const rc = RANK_COLOURS[i % RANK_COLOURS.length];
                return (
                  <div className="c6-radar-dcard" key={d.domain} style={{ ["--rc" as string]: rc } as React.CSSProperties}>
                    <div className="c6-radar-dcard-rk">TOP AREA 0{i + 1}</div>
                    <div className="c6-radar-dcard-nm">{d.domainName}</div>
                    <div className="c6-radar-dcard-mt">
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Top 5 Career Areas to Explore</h2>
          <p className="c6-description">
          ...
          <div className="c6-domains-grid">
            {output.domainAffinities.slice(0, 5).map((d, i) => {
              ...
              const rc = RANK_COLOURS[i % RANK_COLOURS.length];
              return (
                <div key={d.domain} className="c6-domain-card" style={{ ["--rc" as string]: rc } as React.CSSProperties}>
                  {domain && (
                    <div className="c6-domain-img-wrap">
                      <img src={domain.image} alt={d.domainName} className="c6-domain-img" loading="lazy" />
                      <span className="c6-domain-img-rk">{i + 1}</span>
                    </div>
                  )}
                  <div className="c6-domain-body">
                    ...
                    {domain && (
                      <>
                        {domain.skills.length > 0 && (
                          <>
                            <p className="c6-domain-roles-label">Key skills</p>
                            <div className="c6-domain-skills">
                              {domain.skills.slice(0, 6).map((s) => (
                                <span key={s} className="c6-domain-skill">{s}</span>
                              ))}
                            </div>
                          </>
                        )}
                        <p className="c6-domain-roles-label">Roles that fit you here</p>
                        <div className="c6-domain-roles">
                          {domain.roles.slice(0, 8).map((role, ri) => {
                            ...
                            return (
                              <div className="c6-domain-role-row" key={role}>
                                <span className="c6-domain-role-nm">{role}</span>
                                <span className="c6-domain-role-fitwrap">
                                  <span className="c6-domain-role-bar">
                                    <span className="c6-domain-role-fill" style={{ width: `${roleFit}%`, background: rgc }} />
                                  </span>
                                  <span className="c6-domain-role-pct" style={{ color: rgc }}>{roleFit}%</span>
                                </span>
                              </div>
                            );
                          })}
                        </div>
                        <p className="c6-domain-salary">Typical salary (India): {domain.salaryIndia}</p>
                      </>
                    )}
                  </div>
                </div>
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Stay future-proof</h2>
          <p className="c6-description">A quick map of where the world of work is heading, so your choices age well.</p>
          <div className="c6-future-grid">
            <div>
              <div className="c6-future-h">📈 Rising & future-proof</div>
              <div className="c6-future-list">
                {FUTURE.rising.map((x) => (
                  <div className="c6-future-item" key={x.t}><div className="c6-future-t">{x.t}</div><div className="c6-future-d">{x.d}</div></div>
                ))}
              </div>
            </div>
            <div>
              <div className="c6-future-h">📉 Fading or automating</div>
              <ul className="c6-plain-ul">{FUTURE.declining.map((x) => <li key={x}>{x}</li>)}</ul>
              <p className="c6-note">{FUTURE.note}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Your 20-year roadmap</h2>
          <p className="c6-description">
          ...
          {output.domainAffinities[0] && (
            <div className="c6-road-card" style={{ ["--rc" as string]: RANK_COLOURS[0], ["--rc-tint" as string]: RANK_COLOURS[0] + "16" } as React.CSSProperties}>
              <div className="c6-road-intro">
                <span className="c6-road-intro-rk">01</span>
                <div className="c6-road-intro-nm">{output.domainAffinities[0].domainName}</div>
              </div>
              <div className="c6-road">
                {knowledgeRoadmap("6", output.domainAffinities[0].domainName).map((p, i) => (
                  <div className="c6-road-step" key={p.period}>
                    <div className="c6-road-step-num">{i + 1}</div>
                    <div>
                      <div className="c6-road-yr">{p.period}</div>
                      <div className="c6-road-ti">{p.title}</div>
                      <div className="c6-road-tx">{p.points[0]}</div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="c6-road-note">This is one realistic path, not a fixed rulebook — revisit it as your interests sharpen.</div>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">Take the next step</h2>
          <p className="c6-description">Hand-picked starting points relevant to your stage.</p>
          <div className="c6-res">
            {output.domainAffinities[0] && OPPORTUNITIES[output.domainAffinities[0].domain]?.length > 0 && (
              <div className="c6-rgrp">
                <div className="c6-rgh">🚀 Free programs in {output.domainAffinities[0].domainName}</div>
                <div className="c6-rchips">
                  {OPPORTUNITIES[output.domainAffinities[0].domain].map((o) => (
                    <a className="c6-rchip" key={o.url} href={o.url} target="_blank" rel="noreferrer"><b>{o.label}</b><span>{o.note}</span></a>
                  ))}
                </div>
              </div>
            )}
            <div className="c6-rgrp">
              <div className="c6-rgh">📚 Learn these skills — free & paid</div>
              <div className="c6-rchips">{LEARNING.map((l) => <a className="c6-rchip" key={l.url} href={l.url} target="_blank" rel="noreferrer"><b>{l.label}</b><span>{l.note}</span></a>)}</div>
            </div>
            <div className="c6-rgrp">
              <div className="c6-rgh">💼 Where to find jobs & internships (for later)</div>
              <div className="c6-portals">
                {JOB_PORTALS.map((p) => (
                  <div className="c6-pcol" key={p.region}><div className="c6-pr">{p.region}</div><div className="c6-plist">{p.sites.map((si) => <a key={si.url} href={si.url} target="_blank" rel="noreferrer">{si.label}</a>)}</div></div>
                ))}
              </div>
            </div>
            <div className="c6-rgrp">
              <div className="c6-rgh">🎓 Scholarships to know about (2026)</div>
              <div className="c6-schol">
                {SCHOLARSHIPS_2026.map((sc) => (
                  <a className="c6-scard" key={sc.name} href={sc.url} target="_blank" rel="noreferrer"><div className="c6-sn">{sc.name}</div><div className="c6-sw">{sc.who}</div></a>
                ))}
              </div>
            </div>
          </div>
      ...
      <section className="c6-sheet">
        <div className="c6-content">
          <h2 className="c6-section-title">For parents & mentors</h2>
          <p className="c6-description">Supporting {studentName.split(" ")[0] || "your child"} — a few things that help:</p>
          <ul className="c6-plain-ul c6-parent-list">{PARENT_TIPS.map((t, i) => <li key={i}>{t}</li>)}</ul>
        </div>
      </section>
      ...
      <section className="c6-sheet c6-footer">
        <div className="c6-content">
          ...
          <div className="c6-dl-cta">
            <div className="c6-dl-cta-label">Keep a copy</div>
            <div className="c6-dl-cta-row">
              <Class6DownloadButton format="pdf" name={studentName} />
              <Class6DownloadButton format="html" name={studentName} />
            </div>
          </div>
          <div className="c6-contact-block">
            <img className="c6-contact-logo" src="/onegrasp-logo-tight.png" alt="OneGrasp" />
            <div className="c6-contact-lines">
...
function Class6DownloadButton({ format: fmt, name }: { format: "pdf" | "html"; name?: string }) {
  const handleDownload = () => {
    const element = document.querySelector(".class6-report");
    if (!element) { alert("Report not found"); return; }
    const filename = `${name || "Class6-Report"}-${new Date().toISOString().split("T")[0]}`;
    if (fmt === "html") {
      const htmlContent = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${filename}</title><style>body{font-family:system-ui,sans-serif;margin:20px}.class6-report{max-width:900px;margin:0 auto}</style></head><body>${element.outerHTML}</body></html>`;
  ...
  return (
    <button onClick={handleDownload} className="c6-dl-btn">
      <span className="c6-dl-btn-icon">{icon}</span>
      <span>
        <span className="c6-dl-btn-label">{label}</span>
        <span className="c6-dl-btn-desc">{desc}</span>
      </span>
    </button>
  );
}
...
const Class6CSS = `
  ...
  @media (max-width: 860px) {
    .c6-sheet { min-height: auto; }
  }

  @media print {
    .c6-sheet { box-shadow: none; border-radius: 0; margin: 0 auto; }
  }
  ...
  .c6-chip.c6-cc1 { border-left-color: #12996b; }
  .c6-chip.c6-cc3 { border-left-color: #e08a1e; }
  ...
  .c6-scorecard-pill.hi { background: #eaf6f0; color: #1f7a55; }
  .c6-scorecard-pill.mid { background: #fdf3dd; color: #a3620b; }
  .c6-scorecard-pill.lo { background: #f4f4f6; color: #63636f; }
  ...
  @media (max-width: 720px) {
    .c6-radar-hero { grid-template-columns: 1fr; }
  }
  ...
  /* Contents / TOC */
  .c6-toc { display: flex; flex-direction: column; gap: 4px; margin-top: 10px; }
  .c6-toc-row { display: grid; grid-template-columns: 220px 1fr; gap: 14px; padding: 10px 4px; border-bottom: 1px solid #ececef; }
  .c6-toc-t { font-size: 13.5px; font-weight: 700; color: #141417; }
  .c6-toc-d { font-size: 12.5px; color: #63636f; }
  .c6-dims8 { margin: 20px 0 0; border: 1px solid #ececef; border-radius: 14px; overflow: hidden; background: #f7f7f8; }
  .c6-dims8 img { width: 100%; display: block; }

  /* Plain bullet list, shared by future/parents sections */
  .c6-plain-ul { margin: 0; padding-left: 20px; font-size: 13.5px; color: #141417; line-height: 1.85; }
  .c6-parent-list { margin-top: 12px; }

  /* Future outlook */
  .c6-future-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-top: 10px; }
  @media (max-width: 720px) { .c6-future-grid { grid-template-columns: 1fr; } }
  .c6-future-h { font-size: 13.5px; font-weight: 800; margin-bottom: 12px; color: #141417; }
  .c6-future-list { display: flex; flex-direction: column; gap: 10px; }
  .c6-future-item { padding: 10px 13px; border: 1px solid #ececef; border-radius: 10px; background: #f7f7f8; }
  .c6-future-t { font-size: 13px; font-weight: 700; color: #141417; }
  .c6-future-d { font-size: 12px; color: #63636f; margin-top: 3px; }

  /* 20-year roadmap */
  .c6-road-card { background: var(--rc-tint, #f7f7f8); border: 1px solid var(--rc-line, #ececef); border-radius: 20px; padding: 26px 22px; margin-top: 10px; }
  .c6-road-intro { display: flex; align-items: center; gap: 14px; margin-bottom: 20px; padding-bottom: 16px; border-bottom: 1px solid var(--rc-line, #ececef); }
  .c6-road-intro-rk { width: 34px; height: 34px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 13px; color: #fff; background: var(--rc, #F2555A); flex: none; }
  .c6-road-intro-nm { font-size: 17px; font-weight: 800; color: #141417; }
  .c6-road { position: relative; padding-left: 36px; }
  .c6-road::before { content: ""; position: absolute; left: 11px; top: 4px; bottom: 4px; width: 2px; background: rgba(0,0,0,.1); }
  .c6-road-step { position: relative; display: flex; gap: 14px; margin-bottom: 18px; }
  .c6-road-step:last-child { margin-bottom: 0; }
  .c6-road-step-num { position: absolute; left: -36px; top: -2px; width: 24px; height: 24px; border-radius: 50%; background: var(--rc, #F2555A); color: #fff; font-size: 11px; font-weight: 800; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 0 4px var(--rc-tint, #f7f7f8); }
  .c6-road-yr { font-size: 10.5px; font-weight: 800; letter-spacing: 0.05em; text-transform: uppercase; color: var(--rc, #F2555A); }
  .c6-road-ti { font-size: 14px; font-weight: 800; color: #141417; margin-top: 2px; }
  .c6-road-tx { font-size: 12.5px; color: #3d3d45; margin-top: 4px; line-height: 1.55; }
  .c6-road-note { margin-top: 16px; padding: 11px 14px; background: #fff; border: 1px solid var(--rc-line, #ececef); border-radius: 10px; font-size: 11.5px; color: #63636f; font-style: italic; }

  /* Take the next step */
  .c6-res { display: flex; flex-direction: column; gap: 20px; margin-top: 10px; }
  .c6-rgh { font-size: 13.5px; font-weight: 800; margin-bottom: 11px; color: #141417; }
  .c6-rchips { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 9px; }
  @media (max-width: 640px) { .c6-rchips { grid-template-columns: 1fr 1fr; } }
  @media (max-width: 440px) { .c6-rchips { grid-template-columns: 1fr; } }
  .c6-rchip { display: block; border: 1px solid #ececef; border-radius: 11px; padding: 11px 13px; text-decoration: none; background: #fff; }
  .c6-rchip b { display: block; font-size: 13px; font-weight: 800; color: #141417; }
  .c6-rchip span { display: block; font-size: 11px; color: #63636f; margin-top: 2px; }
  .c6-portals { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
  @media (max-width: 560px) { .c6-portals { grid-template-columns: 1fr; } }
  .c6-pcol { border: 1px solid #ececef; border-radius: 12px; padding: 13px 15px; background: #fff; }
  .c6-pr { font-size: 11px; font-weight: 800; letter-spacing: 0.06em; text-transform: uppercase; color: #9a9aa6; margin-bottom: 9px; }
  .c6-plist { display: flex; flex-wrap: wrap; gap: 7px; }
  .c6-plist a { font-size: 12px; font-weight: 700; color: #F2555A; text-decoration: none; background: #FDECED; border: 1px solid #F8CBCD; padding: 6px 11px; border-radius: 8px; }
  .c6-schol { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
  @media (max-width: 560px) { .c6-schol { grid-template-columns: 1fr; } }
  .c6-scard { display: block; border: 1px solid #ececef; border-left: 3px solid #F2555A; border-radius: 11px; padding: 12px 14px; text-decoration: none; background: #fff; }
  .c6-sn { font-size: 13px; font-weight: 800; color: #141417; }
  .c6-sw { font-size: 11px; color: #63636f; margin-top: 3px; line-height: 1.45; }
  ...
  @media (max-width: 560px) {
    .c6-dl-cta-row { grid-template-columns: 1fr; }
  }
  ...
  .c6-dl-btn:hover { border-color: #F8CBCD; }

  .c6-dl-btn-icon { font-size: 22px; flex: none; }
