...
function scorePersonality(responses: Record<string, any>): PersonalityProfile {
  // Questions are answered in order and keyed by index ("0".."9") by
  // convertAnswersToClass11Format. Q0-2 = E/I, Q3-4 = S/N, Q5-6 = T/F,
  // Q7-9 = J/P (see data/class-11-12/questions-corrected.json, personality
  // "11-12" Set 1) — options 0-1 pick the first letter, 2-3 the second.
  let ei = 0, sn = 0, tf = 0, jp = 0;

  for (const q of [0, 1, 2]) ei += responses[q] <= 1 ? 1 : 0;
  for (const q of [0, 1, 2]) ei -= responses[q] >= 2 ? 1 : 0;

  for (const q of [3, 4]) sn += responses[q] <= 1 ? 1 : 0;
  for (const q of [3, 4]) sn -= responses[q] >= 2 ? 1 : 0;

  for (const q of [5, 6]) tf += responses[q] <= 1 ? 1 : 0;
  for (const q of [5, 6]) tf -= responses[q] >= 2 ? 1 : 0;

  for (const q of [7, 8, 9]) jp += responses[q] <= 1 ? 1 : 0;
  for (const q of [7, 8, 9]) jp -= responses[q] >= 2 ? 1 : 0;
