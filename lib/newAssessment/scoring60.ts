...
// MBTI axis letters -> full trait names. The full names are the vocabulary
// the career library's affinity table is keyed on (data/career-map-9-10.json).
const MBTI_TRAIT: Record<string, string> = {
...
const PLAIN_TRAIT: Record<string, string> = {
  ...
  Introversion: "energy from quiet, focused time",
  Sensing: "focus on facts and practical detail",
  Intuition: "focus on ideas and possibilities",
  Thinking: "logic and objective analysis",
  Feeling: "values and how choices affect people",
  Judging: "planning and structure",
  Perceiving: "flexibility and staying open-ended",
};
...
export function scoreAssessment60(
  ...
  const mbtiRanked = rank(big.raw, big.max, (k) => MBTI_TRAIT[k] ?? k);
  // Forced choice makes it impossible to score high on all eight traits at
  // once, so the mean is structurally low for everyone and would read as a
  // weakness beside the other spokes. The spoke is the strongest trait: how
  // pronounced the personality profile is.
  const personalityScore = mbtiRanked[0]?.score ?? 0;

  // Per-axis 0-10 scores for the report's MBTI dial: higher = leans toward
  // the axis's first letter (E/S/T/J), lower = the second (I/N/F/P). Raw
  // tally counts (not the 0-100 rank score) so a genuine tie reads as 5, not
  // a rounding artefact.
  ...
  const mbtiTF = axisScore("T", "F");
  const mbtiJP = axisScore("J", "P");
  ...
  const DIM_SCORES: Record<string, Vec> = {
    personality: Object.fromEntries(mbtiRanked.map((x) => [x.name, x.score])),
    aptitude: aptScores,
